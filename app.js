...
function AddToReceipesListOne ( receipeone )
{
  ...
  <span class="id" hidden>${ receipeone.id }</span>
...
<img src=${ receipetwo.image } class="w-100 receipe-image" alt=""/>
 <div class="receipe-body p-2">
 <span class="id"  hidden>${ receipetwo.id }</span>
...
<div class="receipe-body p-2">
<span class="id" hidden>${ receipethree.id }</span>
...
function showreceipesToUI () 
   { 
       ...
       .then( ( receipes ) => 
       { 
           receipesArr = receipes; 
           let collectionOne = receipesArr.splice( 0, 5 ); 
           collectionOne.forEach( ( receipe ) => 
             AddToReceipesListOne( receipe ) ) 
         
           let collectionTwo = receipesArr.splice( 0, 10 ); 
         collectionTwo.forEach( ( receipe ) => AddToReceipesListTwo( receipe ) ) 
         
           let collectionThree = receipesArr.splice( 0, 10 ); 
           collectionThree.forEach( ( receipe ) => AddToReceipesListThree( receipe ) ) 
       } ).catch( (error )=> console.log( error)); 
 }
function saveToCart ( event )
{
  ...
  const id = receipeRow.querySelector( ".id" );
  let receipe = {
    image: image,
    title: title,
  ...
  saveToLocalStorage( receipe );
}
// let localValue = JSON.parse( localStorage.getItem( 'favorites' ) ) || [];
// if ( localValue[ receipe.id ] )
// {
//   console.log( 'already saved' );
//   localValue[ receipe.id ].count++;

// }
// else
// {
//   console.log( '1st time saving' );
//   localValue[ receipe.id ] = {
//     image: receipe.image,
...
//   }
// }
// localStorage.setItem( "favorites", JSON.stringify( localValue ) );
// }

function saveToLocalStorage ( tappedReceipe ) 
{
  let allReceipes = JSON.parse( localStorage.getItem( "favorites" ) ) || [];
  console.log( allReceipes )
  console.log( tappedReceipe)
  for ( let i = 0; i < allReceipes.length; i++ )
  {
    if ( allReceipes[ i ].id !== tappedReceipe.id )
    {
      allReceipes.push( tappedReceipe );
    }
    else
    {
      console.log('same receipe')
    }
